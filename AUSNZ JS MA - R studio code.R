...
#
#   Sterne, J. A. C., Sutton, A. J., Ioannidis, J. P. A., Terrin, N., Jones, D. R., Lau, J., Carpenter, J., Rücker, G., Harbord, R. M., Schmid, C. H., Tetzlaff, 
#               J., Deeks, J. J., Peters, J., Macaskill, P., Schwarzer, G., Duval, S., Altman, D. G., Moher, D., & Higgins, J. P. T. (2011). 
#               Recommendations for examining and interpreting funnel plot asymmetry in meta-analyses of randomised controlled trials. 
#               BMJ, 343(7818), c4737-4307. https://doi.org/10.1136/bmj.d4002 
#
...
prop_fp.z.p.value <- sub("0", " ", format.pval(2 * pnorm(abs(prop_recid_analysis$zi), lower.tail = FALSE), eps = .001, 2))
prop_fp.pooled_OR_z_p_value <- sub("0", "", sprintf("%.3f",prop_recid_analysis_re$pval))
...
prop_fp_authors <- str_replace_all(prop_recid_analysis$authors, "&", "and")
...
if (prop_fp.pooled_OR_Q_p < .05){grid.text(substitute(paste('Model: ', a, ' = ', b, ', ', c, ' < .05, ',d, ' = ', e), list(a = ~ italic('Q'),  b=prop_fp.pooled_OR_Q, c = ~ italic('p'), d= ~ italic('I')^{2}, e=prop_fp.pooled_OR_I2)), # add the model fit stats to the forest plot
                                           x, y, just = "left", gp = gpar(fontsize=7, font = 1))}
if (prop_fp.pooled_OR_Q_p >= .05){grid.text(substitute(paste('Model: ', a,' = ', b, ', ',c, ' = ',d, ', ',e, ' = ', f), list(a = ~ italic('Q'), b=prop_fp.pooled_OR_Q, c = ~ italic('p'), d=prop_fp.pooled_OR_Q_p, e= ~ italic('I')^{2}, f=prop_fp.pooled_OR_I2)), #this line if Q_p >= .05
                                            x, y, just = "left", gp = gpar(fontsize=7, font = 1))}
...
mean_fp.z.p.value <- sub("0", " ", format.pval(2 * pnorm(abs(mean_recid_analysis$zi), lower.tail = FALSE), eps = .001, 1))
mean_fp.pooled_IRR_z_p_value <- sub("0", " ", round2(mean_recid_rma_glmm_CMEL$pval, 3))
...
###Amend the year and author observations
mean_fp_authors <- str_replace_all(mean_recid_analysis$authors, "&", "and")
...
#g <- grid.locator('npc')
x <- unit(0.066, 'npc') #x <- unit(0.05, 'npc')
y <- unit(0.056, 'npc') #y <- unit(0.058, 'npc')
grid.text(substitute(paste('Model:', a,' = ',b,', credibility interval: (',c,', ',d,')' ), list(a= ~ italic('I')^{2}, b=mean_fp.pooled_IRR_I2, c= mean_fp.pooled_IRR_CR_low, d=mean_fp.pooled_IRR_CR_upp)), # add the model fit stats to the forest plot
          x, y, just = "left", gp = gpar(fontsize=7, font = 1))
...
funnel(mean_recid_rma_glmm_CMEL)
# Note: the mean_recid_rma_glmm_CMEL funnel plot has not been tested for asymmetry due to the small sample of effect sizes (see Sterne, et al. 2011, p. 4)
...
mean_recid_analysis$year_recode <- as.numeric(ifelse(mean_recid_analysis$year ==  'n.d.', '2020', mean_recid_analysis$year)) #note the decision was made to amend the year for Study ID 4b to 2020. This only affects the IRR moderator analysis and so is amended here only.
